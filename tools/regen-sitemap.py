...
def render(urls, existing):
    ...
    for url in urls:
        ...
        # Preserve <lastmod>. Historically this function dropped it, silently
        # wiping every value; scripts/update-sitemap-lastmod.py then had to be
        # re-run or the data was lost. New URLs get no lastmod here and are
        # filled in by that script, which main() enforces before returning.
        lm = f"<lastmod>{escape(cur_last)}</lastmod>" if cur_last else ""
        out.append(f'  <url><loc>{escape(url)}</loc>{lm}<changefreq>{freq}</changefreq><priority>{prio}</priority></url>')
    out.append('</urlset>')
    out.append('')
    return "\n".join(out)
...
def main():
    ...
    if args.apply:
        ...
        n_lm = sitemap.count("<lastmod>")
        if n_lm < n_loc:
            print(f"\nGUARD: {n_loc - n_lm} URL(s) have no <lastmod>.",
                  file=sys.stderr)
            print("Run: python3 scripts/update-sitemap-lastmod.py",
                  file=sys.stderr)
            return 1
        return 0
